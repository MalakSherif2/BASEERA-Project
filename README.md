...
> The diagram reflects the components implemented in `src/pipeline.py`. The Temporal Transformer and Spatio-Temporal Fusion stages have corresponding model weight files in `models/`, but their integration into the live inference loop should be verified against the current state of `src/pipeline.py` before being described as fully active in production runs.
...
2. **Pose Detection & Tracking** — YOLO11-Pose (`yolo11n-pose.pt`) detects people frame-by-frame; ByteTrack persists identities across frames, assigning a stable `person_id`.
3. **Keypoint Sequencing** — For each tracked person, 2D keypoints (34 values: 17 keypoints × x,y) are accumulated into a per-person temporal sequence.
4. **Weapon Detection** — A custom-trained YOLO model (`weapon_detect_best.pt`) scans each frame for weapons.
5. **Weapon–Person Association** — Detected weapon boxes are matched to the nearest tracked person whose bounding box contains the weapon's center point, rather than flagging every person in frame.
6. **Behavior Classification** — Once a person has enough accumulated keypoint frames, the LSTM behavioral classifier (`temporal_behavior_model.pt`) predicts a behavior label (`Fighting`, `Robbery`, `Stealing`, or `Normal Movement`) with a confidence score over a sliding window.
7. **Segmentation (conditional)** — If SAM ViT-B is loaded and a weapon association is active, segmentation masks are generated periodically to visually highlight the relevant region.
8. **Severity Scoring** — Each tracked person's final record is scored (`CRITICAL`, `HIGH`, `MEDIUM`, `LOW`) using a rule-based function that considers weapon detection and behavior confidence.
9. **Event Aggregation** — Results are consolidated into `events/event_history/parsed_events.json`, and (when the Advanced Event Engine is active) `advanced_event_history.json`.
10. **Dashboard Rendering** — The Streamlit dashboard loads the JSON output and renders KPIs, threat level, annotated video, per-person cards, and event timelines.
11. **AI Security Analyst (optional)** — If a Gemini API key is configured, the dashboard can query Gemini with the machine-generated evidence to produce event explanations, answer free-form security questions, and generate a full incident report.
...
| **ByteTrack** | Assigns and persists a stable identity per person across frames | `person_id` with continuous tracking |
...
| **Temporal Transformer** | Alternative temporal model for behavior classification, intended to complement the LSTM | Behavior prediction (model weights present in `models/temporal_transformer_model.pt`) |
| **Spatio-Temporal Fusion** | Combines spatial and temporal cues for behavior/threat interpretation | Fused behavior/confidence signal (model weights present in `models/spatiotemporal_fusion_model.pt`) |
...
| **Weapon–Person Association** | Matches detected weapons to the most likely person using bounding-box containment and nearest-distance resolution | Per-person `weapon_detected` flag and `weapon_frames` count |
| **SAM ViT-B** | Generates a segmentation mask for persons associated with a detected weapon | Overlay mask on annotated video output |
| **Event Intelligence Engine** | Aggregates per-frame detections into structured, severity-ranked events | JSON event records (`parsed_events.json`, `advanced_event_history.json`) |
| **Gemini Security Analyst** | Interprets the generated evidence in natural language, strictly grounded in the telemetry provided | Event explanations, chat answers, incident reports |
...
| Field | Description |
|---|---|
| `person_id` | Stable ByteTrack identity |
| `detected_behavior` | Best behavior label observed across the track (LSTM sliding-window output) |
| `behavior_confidence` | Confidence associated with the reported behavior |
| `weapon_detected` | Whether a weapon was ever associated with this person |
| `weapon_frames` / `weapon_detection_ratio` | Number and proportion of frames where a weapon was associated |
| `start_time_sec` / `end_time_sec` / `duration_sec` | Timeline of the person's presence in the video |
| `severity` | Final rule-based severity classification |
| `transformer_behavior` / `transformer_confidence` | Displayed in the dashboard when the Temporal Transformer output is available |
| `fusion_behavior` / `fusion_confidence` | Displayed in the dashboard when the Spatio-Temporal Fusion output is available |

The dashboard renders each tracked person as an individual card summarizing this information, color-coded by severity.
...
VISIONGUARD integrates Google's Gemini API (via the `google-genai` SDK) as an **optional** natural-language security analyst layered on top of the machine-generated evidence. It does not replace the detection pipeline — it only interprets the JSON telemetry the pipeline already produced.
...
![VISIONGUARD Dashboard](AI_Pipeline_Architecture.png)
*The main command center: header, system status, video upload, and AI stack status in the sidebar.*

**Threat Intelligence Overview**
![Threat Intelligence Overview](Threat_Intelligence_Overview.png)
*Frame count, tracked persons, weapon detection frames, critical events, and model prediction counts at a glance.*

**Person Intelligence**
![Person Intelligence](Person_Intelligence.png)
*Per-person cards showing behavior, confidence, weapon association, timeline, and multi-model outputs.*

**Advanced Event Intelligence**
![Event Intelligence](Advanced_Event_Intelligence.png)
*Severity-ranked event feed generated from the Advanced Event Engine.*

**Multi-Model Comparison**
![Model Comparison](Multi-Model_Comparison.png)
*Side-by-side prediction counts for LSTM, Temporal Transformer, and Spatio-Temporal Fusion.*

**Gemini Security Analyst**
![Gemini Analyst](Gemini_Security_Analyst.jpg)
*Event-specific analysis and free-form security chat grounded in pipeline telemetry.*

**Incident Report**
![Incident Report](Incident_Report.png)
*AI-generated, structured incident report with export to `.txt`.*
...
1. Open the sidebar and upload a surveillance video (`.mp4`, `.avi`, `.mov`, `.mkv`, `.webm`, or `.mpeg4`).
2. Click **"🚀 RUN FULL AI ANALYSIS"**.
3. Wait for the pipeline to process the video and generate outputs.
4. Review the Threat Intelligence Overview, annotated video, person cards, and events.
5. Optionally, use the Gemini Security Analyst to query the results or generate an incident report.
...
> ⚠️ Docker configuration should be verified against the current `Dockerfile` / `docker-compose.yml` contents (e.g. exposed ports, mounted volumes, and whether GPU passthrough is configured) before relying on it for deployment.
